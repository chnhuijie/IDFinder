    ...
    if missing_players:
        ...
        for player in missing_players:
            ...
            if club_details["rank"] < 250:
                top250_leavers.append({"id": player.get("mid"), "name": player.get("name", "Unknown"), "old_club": player.get("club"), "old_club_id": club_id, "rank": club_details["rank"] + 1})
            ...
            if len(dropped_clubs) > 10:
                ...
                if dropped_clubs:
                    ...
                    for club_id, club_name in dropped_clubs:
                        messages.append(f"  • **{club_name}** | Lost tracking for {leaver_counts[(club_id, club_name)]} players.")
                if individual_leavers:
                    messages.append("**Top 250 Club Leavers Detected**")
                    for l in sorted(individual_leavers, key=lambda x: x['rank']):
                        messages.append(f"  • `ID: {l['id']}` | **{l['name']}** left **{l['old_club']}**")

        if new_players:
            messages.append(f"**{len(new_players)}** new players entered the tracking pool.")
            new_clubs = Counter((p.get("club"),) for p in new_players)
            for (club_name,), count in new_clubs.most_common(15):
                messages.append(f"  • **{club_name}**: +{count} new")

        if transfers:
            messages.append(f"**{len(transfers)}** players moved between tracked clubs.")
            shift_clubs = Counter((p.get("club"),) for p in transfers)
            for (club_name,), count in shift_clubs.most_common(15):
                messages.append(f"  • **{club_name}**: +{count} transferred")
    else:
