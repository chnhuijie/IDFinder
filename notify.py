...
def process_post_scan_transfers():
    ...
    if missing_players:
        ...
        for player in missing_players:
            ...
            # Anti-False Alarm: Ignore missing players if their club failed to scan entirely.
    ...
    if is_api_outage:
        ...
        for l in top500_leavers_raw:
            if (l['old_club_id'], l['old_club']) in dropped_clubs:
                # If a whole club disappears, delete them to clear the tracking pool
                bulk_updates.append(DeleteOne({"_id": l["_id"]}))
            elif l['id'] in known_botters:
                # Silently un-track known botters without hitting the Discord Webhook
                bot_purge_ids.append(l['_id'])
                bulk_updates.append(UpdateOne({"_id": l["_id"]}, {"$set": {"club": None, "club_id": None, "club_tier": "Unranked", "previous_club": None, "previous_club_id": None}}))
            else:
                individual_leavers.append(l)
                # Formats precisely for Bamboo's /cl Queue
    ...
    print(f"Normal Top 500 Leavers Sent to Bamboo /cl: {len(individual_leavers)}")
    print(f"Known Botters Silently Purged: {len(bot_purge_ids)}")
    print(f"New Players Entered Tracking Pool: {len(new_players)}")
    print(f"Transfers Detected Between Clubs: {len(transfers)}")
    print("==============================\n")
        ...
        if top500_leavers_raw:
            if is_api_outage:
                ...
                if individual_leavers:
                    messages.append("**Top 500 Club Leavers Detected** *(Awaiting Bamboo /check)*")
