    ...
    for club in target_clubs:
        ...
        if detail and "members" in detail:
            ...
            for m in (detail.get("members") or []):
                ...
                # If player isn't in database, they are brand-new
                if not prev_record:
                    new_count += 1
                # If they are in DB but club changed, track it as a shift
                elif prev_record.get("club") != club_name:
                    shift_count += 1

                # upsert=True updates existing data or safely adds them if missing
                ops.append(UpdateOne(
                    {"mid": p_id},
                    {"$set": {
                        "name": p_name, 
                        "club": club_name, 
                        "last_seen": time.time()  # High-resolution time hook
                    }},
                    upsert=True
                ))
